...
**Correct: memoize (dynamic query) or hoist (static)**

```tsx
// Dynamic query – memoize inside component
function Highlighter({ text, query }: Props) {
...
// Static pattern – hoist outside component
const staticRegex = /pattern/gi; // module scope

function Highlighter({ text }: Props) {
  const parts = text.split(staticRegex);
  return <>{parts.map((part, i) => ...)}</>;
}
...
```typescript
// Mutating props/state can break memoization, cause infinite loops, or lead to stale UI.
```

**Browser support:**

```typescript
// Browser support fallback (if needed):
